...
class MosaicDetection(Dataset):
    ...
    def mixup(self, origin_img, origin_labels, input_dim):
        ...
        num_channels = origin_img.shape[2]

        if len(img.shape) == 3:
            # Use the dynamic channel count
            cp_img = (
                np.ones((input_dim[0], input_dim[1], num_channels), dtype=np.uint8)
                * 114
            )
        else:
        ...
        padded_img = np.zeros(
            (max(origin_h, target_h), max(origin_w, target_w), num_channels),
            dtype=np.uint8,
        )
